...
{
	/// <summary>
	/// An XSLT Mediator allowing the use of XSLT TBBS as part of compound templates
	/// </summary>
	/// <author>Created by Chris Summers, Frank van Pufflen and Yoav Niran (SDL Tridion)</author>
	...
	public class XsltMediator : IMediator
	{
		...
		protected TemplatingLogger Logger { get { return m_Logger; } }
		...
		private void AddPositioningVariables(XsltArgumentList args)
		{			
			var ordPos = 0;
			var regPos = 0;

			//CS: Looks for an ORDINAL_POSITION package item and adds the number, otherwise adds it as a 0
			//Probably not really needed
			if (m_Engine.PublishingContext != null)
			{
				var context = m_Engine.PublishingContext.RenderContext;

				if (context != null)
				{
					if (context.ContextVariables.Contains(ORDINAL_POSITION))
					{
						int.TryParse(m_Engine.PublishingContext.RenderContext.ContextVariables[ORDINAL_POSITION].ToString(), out ordPos);						
					}

					if (context.ContextVariables.Contains(REGIONAL_POSITION))
					{
						int.TryParse(m_Engine.PublishingContext.RenderContext.ContextVariables[REGIONAL_POSITION].ToString(), out regPos);
					}
				}
			}

			args.AddParam(ORDINAL_POSITION, String.Empty, ordPos);
			args.AddParam(REGIONAL_POSITION, String.Empty, regPos);
		}

		private XsltArgumentList CreateArgumentCollection()
		{
			...
			if (m_GetFromPackage) // Loop through all of the entries in the package, and add them argument list as parameters
			{
				...
				foreach (KeyValuePair<String, Item> pair in packageEntries)
				{
						...
						if (!addedKeys.Contains(key)) //package may contain duplicate keys
						{
							switch (pair.Value.ContentType.Value)
							{
								...
								default: //Other items are added as XmlDocuments								
									args.AddParam(key, "", pair.Value.GetAsXmlDocument());
					...
					{								//look for a constructor with no parameters
						...
						{							//look for a constructor that accepts the Engine as the only parameter
							...
							else
							{
								...
								{
									Logger.Warning(String.Format("type: '{0}' doesnt have a valid constructor!", t.FullName));
								}
							}
						}
		...
		private void ReadParameters()
		{
			...
			if (string.IsNullOrEmpty(m_InputItemName))
			{
				...
				if (m_XmlInput == null) //probably wont ever happen...
					m_Logger.Warning("Amazingly... there is no InputItem, Component or Page, which is a  bit of a mystery");
				else
					m_Logger.Debug("Using a Page as the input item");
			}
			else
			{
				...
				if (m_XmlInput == null)
				{
					m_Logger.Warning("Couldnt find package item with name: " + m_InputItemName);
